...
def process_email_body_advanced(body):
    doc = ner_pipeline(body)

    extracted_info = {
        "Pick up date": None,
        "Drop off date": None,
        "logistic_shipment_pickup_location": None,
        "logistic_shipment_dropoff_location": None,
    }

    pu_date_match = re.search(r'PU-\s*(\d+/\d+)', body)
    if pu_date_match:
        extracted_info["Pick up date"] = pu_date_match.group(1)
    
    del_date_match = re.search(r'DEL-\s*(\d+/\d+)', body)
    if del_date_match:
        extracted_info["Drop off date"] = del_date_match.group(1)

    location_pattern = re.search(r"\n\n([\w\s]+), (\w\w) to ([\w\s]+), (\w\w)", body)
    if location_pattern:
        extracted_info["logistic_shipment_pickup_location"] = f"{location_pattern.group(1).strip()}, {location_pattern.group(2)}"
        extracted_info["logistic_shipment_dropoff_location"] = f"{location_pattern.group(3).strip()}, {location_pattern.group(4)}"
...
extracted_info = process_email_body_advanced(sample_email_content)
